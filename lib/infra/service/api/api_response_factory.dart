...
import 'model/qiita_error_json.dart';
import 'network_exceptions.dart';

final apiResponseFactoryProvider = Provider<ApiResponseFactory>((ref) => ApiResponseFactory());
...
class ApiResponseFactory {
  ...
  ApiResponse<T> _handleDioError<T>(DioError error, StackTrace stackTrace) {
    if (error.response?.data is Map<String, dynamic>) {
      ...
      final errorJson = QiitaErrorJson.fromJson(data);
      return ApiFailureResponse(NetworkExceptions.getDioException(error, message: errorJson.message));
    }

    return ApiFailureResponse(NetworkExceptions.getDioException(error));
  }

  Future<ApiResponse<T>> handleExpectedException<T>(Response<dynamic> res) async {
